...
int main(int argc, char *argv[]) {
	...
	if (db){
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(s.name[0] != 'a' && s.name[0] != 'A' && s.name[0] != 'e' && s.name[0] != 'E' && s.name[0] != 'i' && s.name[0] != 'I' && s.name[0] != 'o' && s.name[0] != 'O' && s.name[0] != 'u' && s.name[0] != 'U'){ // *** first filter, conditions on the student
